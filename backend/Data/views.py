...
class data(View):
    def post(self, request):
        ...
        try:
            ...
            ori_data_url = ossUtil.upload(data_type +"/", date_description, file)
...
class ori_data(View):
    def get(self, request):
        try:
            ...
            if user_id is None:
                return JsonResponse({"status": "error", "message": "Authentication required"}, status=401)
            ori_datas = Data.objects.filter(user_id=user_id).values()
...
class cleaned_data(View):
    def get(self, request):
        try:
            ...
            if user_id is None:
                return JsonResponse({"status": "error", "message": "Authentication required"}, status=401)
            cleaned_datas = Data.objects.filter(user_id=user_id).exclude(cleaned_data_url='').values()
...
class marked_data(View):
    def get(self, request):
        try:
            ...
            if user_id is None:
                return JsonResponse({"status": "error", "message": "Authentication required"}, status=401)
            marked_datas = Data.objects.filter(user_id=user_id).exclude(marked_data_url='').values()
...
class preprocessed_data(View):
    def get(self, request):
        try:
            ...
            if user_id is None:
                return JsonResponse({"status": "error", "message": "Authentication required"}, status=401)
            preprocessed_datas = Data.objects.filter(user_id=user_id).exclude(preprocessed_data_url='').values()
...
class marked_preprocessed_data(View):
    def get(self, request):
        try:
            ...
            if user_id is None:
                return JsonResponse({"status": "error", "message": "Authentication required"}, status=401)
            marked_preprocessed_datas = Data.objects.filter(user_id=user_id).exclude(marked_preprocessed_data_url='').values()
