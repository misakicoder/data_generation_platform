<template>
    <div>
        <div class="relative w-full" v-if="current_task.videos[props.index]">
            <video controls class="w-72 h-auto rounded-lg inline-block">
                <source :src="s3_video_path + current_task.videos[props.index]" type="video/mp4" />
            </video>
        ...
        <div class="flex justify-center items-center" v-else>
            <div class="flex animate-pulse w-72 opacity-80">
                <div
                    ...
                    <button class="flex flex-auto flex-col justify-center items-center p-4 md:p-5"
                        @click="regenerate_me" :disabled="!current_task.scenes[index]"
                        :class="{ 'hover:scale-105 cursor-pointer': current_task.scenes[index] }">
                        <i class="ti ti-slideshow text-xl"></i>
                        <span class="text-xs">生成分镜视频</span>
                    </button>
                </div>
            </div>
        </div>
...
import { s3_video_path } from '@/util/api';
