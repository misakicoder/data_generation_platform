    ...
    <div>
        <div v-if="current_task.movie" class="relative">
            <video controls class="w-full h-auto rounded-lg inline-block">
                <source :src="s3_movie_refined_path + current_task.movie" type="video/mp4" />
            </video>
        ...
        <div class="flex animate-pulse w-full opacity-80" v-else>
            <div
                ...
                <button
                    ...
                    @click="generate_movie" :disabled="!current_task.movie"
                    :class="{ 'hover:scale-105 cursor-pointer': current_task.movie }">
                    <i class="ti ti-movie text-xl"></i>
                    <span class="text-xs">超分插帧</span>
                </button>
            </div>
        </div>
    </div>
...
import { s3_movie_refined_path } from '@/util/api';
